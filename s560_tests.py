...
def test_ship_attrs():
    ...
    assert (
        ship.length == 3
        and ship.tp == "vertical"
        and ship.coords_init.x == 0
        and ship.coords_init.y == 0
    ), "неверные значения атрибутов объекта класса Ship"
    assert ship.tp == "vertical", "неверное значение атрибута tp"


def test_ship_cells_and_move(ship4v: Ship):
    ship = ship4v
    assert ship.cells == [1, 1, 1], "неверный список _cells"
    assert ship.is_move, "неверное значение атрибута _is_move"
...
def test_ship_set_start_coords(ship4v: Ship):
    ...
    assert (
        ship.coords_init.x == 1 and ship.coords_init.y == 2
...
def test_ship3h_around():
    ...
    left = {(0, 1)}
    right = {(4, 1)}
    assert s1.around == above.union(below, left, right)
